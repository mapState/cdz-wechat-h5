...
import { Button, Field, Toast, Dialog, Empty } from 'vant';
[Button, Field, Toast, Dialog, Empty].forEach(e => {
    Vue.use(e)
});
...
axios.defaults.baseURL = siteinfo.siteroot + 'index.php?s=/api/';
...
axios.interceptors.response.use(function(response) {
    ...
    if (res.code === 1) {
        ...
        Toast.fail(res.msg || res.code)
        return Promise.reject(res);
    }
}, (error) => {
    let res = error;
    Toast.fail(res.errMsg || res.status)
    return Promise.reject(error);
});
